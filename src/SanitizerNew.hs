...
processTree :: Text -> Text
processTree = renderTree . modifyTree canProcess (toList . cleanTree) . parseTree

cleanTree :: TagTree Text -> Maybe (TagTree Text)
cleanTree =
  removeTag "script" >=>
  removeTag "style" >=>
  removeTag "iframe" >=> removeWS >=> removeStylesheets >=> removeShare
...
modifyTag name f t =
  if (t ^. ttname) == name -- TODO ensure case insensitive here
    then f t
    else Just t

removeWS :: TagTree Text -> Maybe (TagTree Text)
removeWS t =
  if t ^. ttname & (T.unwords . T.words) & T.null
    then Nothing
    else Just t
...
removeStylesheets :: TagTree Text -> Maybe (TagTree Text)
removeStylesheets t =
  if (t ^. ttname == "link") && ("rel" `attrHas` "stylesheet" $ t)
    then Nothing
    else Just t

removeShare :: TagTree Text -> Maybe (TagTree Text)
removeShare =
  "class" `skipIfAttrHas` "feedflare" >=>
  "class" `skipIfAttrHas` "mediafed_ad" >=> "id" `skipIfAttrHas` "jp-post-flair"

keepIfAttrHas name val t =
  if attrHas name val t
    then Just t
    else Nothing

skipIfAttrHas name val t =
  if attrHas name val t
    then Nothing
    else Just t

attrHas :: Text -> Text -> TagTree Text -> Bool
attrHas name value t = t ^. ttattr & lookup name & hasValue value
  where
    hasValue _ Nothing = False
    hasValue v (Just a) = T.isInfixOf v a -- TODO: ensure case insensitive
...
ttattr f (TagBranch n attr t) = (\a -> TagBranch n a t) <$> f attr
